...
<div>
	...
	<div id="nav">
		<router-link to="/" @click="runningTest = false">Документация</router-link> |
...
export default {
	data: () => ({
		...
		test: [
			{ question: 'кто', correctAnswer: 'я', answer: null },
			{ question: 'привет', correctAnswer: 'да', answer: null },
		],
		uname: '',
		ugroup: ''
	}),
	methods: {
		...
		async startTest(train) {
			...
			let typeCount = null;
			if (train) {
				typeCount = (prompt('Введите данные для теста в формате (основание системы счисления >= 2! & <= 16): "основание_системы_счисления,количество_заданий;основание_системы_счисления,количество_заданий;..."') || '')
				.split(';').map(v => v.split(',').map(v2 => parseInt(v2)));
				for (let i = typeCount.length - 1; i >= 0; i--)
					if (typeof typeCount[i] != 'object' || typeCount[i].length < 2 || typeCount[i][0] < 2)
			...
			this.test = await eel.GenTest(this.uname, this.ugroup, train, typeCount)();
